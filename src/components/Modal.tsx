...
function Modal({data, tests, setState}: {data: { id: number; due_date: Date; subject: string; news: boolean; desc?: string; content?: string[]; hasInstructions?: boolean; fileID?: string }[], tests?: boolean, setState: React.Dispatch<React.SetStateAction<boolean>>}) {
            ...
            <div className="modal__background">
                <div className="modal__container">
                    ...
                    <div className="modal__title mb-5">
                        ...
                        <p>{data.filter((item)=>item.news) ? 'Um novo item foi adicionado a lista. Fique atento!' : 'Não hove nenhuma atualização desde a última checagem.'}</p>
                    </div>
                        ...
                        {data.length !== 0 ? <ul className='flex flex-column gap-3'>
                            {data.slice().sort((a,b) => Number(b.news) - Number(a.news)).map((item) => (<li key={item.id}>
                                ...
                                <div className={`flex ${item.news ? 'justify-content-between' : 'justify-content-end'}`}>
                                    ...
                                    {item.hasInstructions && <DownloadButton fileID={item.fileID ?? undefined}>Baixar {tests ? 'conteúdo' : 'enunciado'}</DownloadButton>}
                                </div>
                            </li>))}
