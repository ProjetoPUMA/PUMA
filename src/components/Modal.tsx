...
function Modal({data, tests, setState, works}: {data: { id: number; due_date: Date; subject: string; news: boolean; desc?: string; content?: string[]; hasInstructions?: boolean; fileID?: string; title?: string }[], tests?: boolean, setState: React.Dispatch<React.SetStateAction<boolean>>, works?: boolean}) {
    ...
    const expired_array = data.filter((item)=> isBefore(item.due_date, new Date()) && !isToday(item.due_date))
    const notExpired_array = data.filter((item) => isBefore(new Date(), item.due_date) || isToday(item.due_date))
    const used_array = isExpiredClicked ? expired_array : showAll ? data : notExpired_array;
            ...
            <div className="modal__background">
                <div className="modal__container">
                    ...
                    <div className='flex gap-5 justify-content-center pb-5'>
                        ...
                        <span onClick={()=>{setIsExpiredClicked(false); setShowAll(false)}}>Em breve</span>
                        <span onClick={()=>{setIsExpiredClicked(true); setShowAll(false)}}>Expiradas</span>
                    </div>
                    <div className="modal__body">
                        {used_array.length !== 0 ? <ul className='flex flex-column gap-3'>
                            {used_array.slice().sort((a,b) => {
                        ...
                        }).map((item) => (<li className={classNames('modal--item', {'homework--news': item.news, 'homework--expired': isBefore(item.due_date, new Date()) && !isToday(item.due_date), 'homework--attention': isToday(item.due_date)})} key={item.id}>
                                ...
                                : <p className={`${isBefore(item.due_date, new Date()) ? 'mb-0' : 'mb-4'}`}>{item.desc}</p>}
                                ...
                                {(isBefore(item.due_date, new Date()) && !isToday(item.due_date)) ?<small>Essa atividade já passou de sua de vencimento.</small> : isToday(item.due_date) && <small>Hoje é o último dia de prazo desta atividade!</small>}
                            </li>))}
